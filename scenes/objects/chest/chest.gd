...
@export var dialogue_start_comand: String
...
func _unhandled_input(event: InputEvent) -> void:
	if in_range:
		if event.is_action_pressed("show_dialogue"):
			...
			balloon.start(load("res://dialogue/conversations/chest.dialogue"), dialogue_start_comand)
...
func trigger_feed_harvest(inventory_item: String, scene: Resource) -> void:
	...
	for index in inventory_item_count:
		...
		harvest_instance.global_position = Vector2(global_position.x, global_position.y)
